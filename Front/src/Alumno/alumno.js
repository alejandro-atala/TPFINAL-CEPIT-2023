...
const Alumno = () => {
        ...
        if (Array.isArray(avisosData)) {
          ...
          const unreadCount = await Promise.all(
            idsAvisosSinLeer.map(async (idAviso) => {
              try {
                const response = await axios.get(`https://app-2361a359-07df-48b8-acfd-5fb4c0536ce2.cleverapps.io/alumno-aviso/existe/${idAviso}/${alumnoLogueado.idAlumno}`);
  ...
  const marcarAvisosVistos = async () => {
    try {
      ...
      const response = await axios.put(`https://app-2361a359-07df-48b8-acfd-5fb4c0536ce2.cleverapps.io/alumno-aviso/marcar-leidos/${avisoId}/${alumnoLogueado.idAlumno}`);
    ...
    try {
      ...
      for (const idAviso of idsAvisos) {
        ...
        if (!response.data) {
          ...
          const responseGuardar = await axios.post('https://app-2361a359-07df-48b8-acfd-5fb4c0536ce2.cleverapps.io/alumno-aviso', data);
