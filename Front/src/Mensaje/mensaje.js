...
function Mensaje() {
  ...
  useEffect(() => {
    ...
    fetch('https://app-9d7fdcc2-2916-41fd-93f1-ef602d6afbcc.cleverapps.io/usuario')
    ...
    if (!socket && usuarioLogueado?.id) {
      const newSocket = io('https://app-9d7fdcc2-2916-41fd-93f1-ef602d6afbcc.cleverapps.io', {
  ...
  const fetchPreviousMessages = async (senderId, receiverId) => {
    try {
      const response = await fetch(`https://app-9d7fdcc2-2916-41fd-93f1-ef602d6afbcc.cleverapps.io/messages/previous/${senderId}/${receiverId}`);
  ...
  const cerrarChat = () => {
   ...
   const enviarPrimerMensaje = async () => {
    if (chatDestino && nuevoMensaje && usuarioLogueado && usuarioReceptor) {
      ...
      try {
        const response = await fetch('https://app-9d7fdcc2-2916-41fd-93f1-ef602d6afbcc.cleverapps.io/messages', {
          method: 'POST',
          headers: {
            ...
            if (messageToDelete) {
              if (option === 'deleteForMe') {
                ...
                const response = await fetch(
                  `https://app-9d7fdcc2-2916-41fd-93f1-ef602d6afbcc.cleverapps.io/messages/${messageId}`,
                  {
                    method: 'PATCH',
                    headers: {
                      'Content-Type': 'application/json',
